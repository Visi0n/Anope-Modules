...
/* functions */
int fantasy_redirect(int argc, char **argv);
int fantasy_redirect_help(int ac, char **av);
...
int AnopeInit(int argc, char **argv)
{
    ...
    hook = createEventHook(EVENT_BOT_FANTASY, fantasy_redirect);
...
int fantasy_redirect(int argc, char **argv)
{
   ...
   else if ((stricmp(argv[0], "redirect") == 0)) 
   {
        u = finduser(argv[1]);
        ...
        if (argc >= 4) 
        {
            ...
            channel = myStrGetTokenRemainder(argv[3], ' ', 1);
        }
        if (!check_access(u, ci, CA_KICK)) 
        {
            notice(s_BotServ, u->nick, "Only IRCops are allowed to use redirect");
        }
